...
収集対象、収集定義、抽出 revision、状態、取得理由、task/attempt、URL 候補、Realtime/Backfill の優先制御を Resource 中心へ統一する次期設計は [26-collection-platform-design.md](26-collection-platform-design.md) を正本とする。既存収集ジョブ実装は新基盤へ controlled cutover 後に削除し、恒久互換層を残さない方針である。本案は未承認であり、現行動作はまだ変更していない。
