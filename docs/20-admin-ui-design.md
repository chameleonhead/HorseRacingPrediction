...
一覧の補助操作「開催日を再取得」では、JSTの日付を1つ指定し、確認画面で登録済みの同日JRAレースについて基本情報、出馬表、馬主を含む出走情報、公開済み結果・払戻を更新することを示す。送信後は日付親ジョブ詳細へ遷移し、レース単位に分割された進捗と失敗対象を追跡する。同日付の有効な依頼がある場合は重複作成せず、そのジョブへのリンクを表示する。設計と検証は [開催日単位でJRAレースデータを再取得する](changes/20260909_race-day-reacquisition/README.md) を参照。
