...
> 同日の利用者回答で、タイムアウト時は収集全体を停止し、手動保留は単一ジョブのみとする範囲を確定した。改訂案では詳細から未開始ジョブを保留し、実行中ジョブは「保留して中断」で新規リースを拒否してキャンセルする。中断待ち→保留中を区別し、中断確認前の解除は受け付けない。親・子への保留の連鎖は行わない。改訂案の承認・実装は未完了。
