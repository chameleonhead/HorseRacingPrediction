...
ジョブ一覧の全体停止は、後続の配送と新規リース取得を止め、停止状態・理由・発生日時・原因ジョブを永続化する。SQS本体とDLQの消去は行わない。開始済みのジョブは完了・失敗を報告でき、管理画面の通常操作も継続できる。タイムアウト時は当該ジョブを失敗にして収集全体を停止する。明示的な「再開」で実行可能なジョブの配送を再作成するが、失敗ジョブのリランと個別保留の解除は別操作とする。
