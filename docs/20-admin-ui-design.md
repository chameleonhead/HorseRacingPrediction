...
2026-09-15 提案: 一覧の補助操作「収集を依頼」から「期間を指定してレースを再取得」を選べるようにする。JST の開始日と終了日（両端を含む）または「先週末」presetを指定し、JRA公式情報からDB未登録分を含む各日の全レースを探し直す。確認では解決済み期間、対象日数、更新対象、過去オッズが対象外であることを示す。期間は日別taskへ展開し、batchとして進捗を追跡する。既存月次Backfillは未取得補完として維持し、完了済み日も再実行する本操作とは区別する。詳細は[期間を指定してレース情報を再取得する](changes/20260915_race-period-recollection/README.md)を正とし、承認・実装までは現行UIを変更しない。旧単日ジョブの仕様は履歴として保持する。
