...
一覧の補助操作「開催日を再取得」では、JSTの日付を1つ指定し、JRA公式情報からDB未登録分を含む同日の全レースを探し直す。確認画面では、出馬表は公式掲載期間内の日だけ、結果・払戻は当日以前だけ取得し、出馬表を取得できる場合は馬主を含む出走情報も更新することを示す。送信後は日付親ジョブ詳細へ遷移し、レース単位に分割された進捗と失敗対象を追跡する。同日付の有効な依頼がある場合は重複作成せず、そのジョブへのリンクを表示する。設計と検証は [開催日単位でJRAレースデータを再取得する](changes/20260909_race-day-reacquisition/README.md) を参照。
