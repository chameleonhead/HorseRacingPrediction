...
一覧は成功完了していないジョブを先、`Succeeded` の完了済みジョブを後にする。各グループ内では実行基盤の安定した運用優先順位に合わせ、処理区分をリアルタイム、通常、バックグラウンドの順、同じ処理区分では数値優先度の降順、さらに同順位ではジョブ種類を表す Definition ID の昇順で表示する。日本語表示名は並び順に使用しない。すべてが同じ場合は Task ID を安定した最終タイブレークとし、この順序をページング前の全検索結果へ適用する。`Failed`、`DeadLetter`、`Cancelled` は成功完了ではないため未完了側に置く。実配送時だけ使う待機時間による aging と、リアルタイム処理の連続後にバックグラウンド処理を選ぶ飢餓防止は、時刻と配送履歴で変動するため一覧順序には含めない。詳細な変更契約は[収集ジョブ一覧を運用優先順位で並べる](changes/20260914_collection-job-priority-order/README.md)を参照する。
