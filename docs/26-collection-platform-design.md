...
16. 主体プロフィールの同定失敗は、一時アクセス障害、既知の名前正規化、誤生成参照、提供元対象外、曖昧同定、ページ構造エラーへ分類する。同じrevisionの盲目的な再試行は行わず、修正済みの新revisionと決定的な同一性根拠がある対象だけをfailureごとに一度、自動Recoveryできる。提供元対象外と誤生成参照は再取得せず理由付き終端にし、曖昧対象と構造エラーは誤結合せず要対応に残す。詳細契約は[主体識別エラーを分類し安全に自動復旧する](changes/20260918_subject-identification-auto-recovery/README.md)を参照し、承認前は現行動作を維持する。
