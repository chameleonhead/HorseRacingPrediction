...
18. 収集運用監視は同一cutoffの読み取り専用snapshotを使い、要対応失敗、停止、滞留、優先順異常を評価する。順序異常は同一laneかつdispatcher互換性keyが一致するtask間だけを対象とし、独立した配送判断が3回以上追い越した場合に限る。definition別に到着、配送、完了、active、最古age、compatibility keyを同じcutoffで返し、starvation、capability gap、capacity不足、意図的待機を分類する。findingは原因仮説、所有task、次の安全な操作へ集約し、反復ごとのchange recordやPRは作らない。詳細は[収集監視を観測記録から原因分析と実行タスクへ変更する](changes/20260919_collection-monitor-root-cause-triage/README.md)を参照する。
...
閉じたbrowser/page/contextは構造・データ整合性障害ではなくsession-level transientとして扱う。実行scopeでfresh sessionを一度だけ試し、それでも失敗したattemptはcanonical `TargetClosedException` として遅延再配送する。ただし既定10分窓で3件に達した場合はbrowser基盤の広域障害とみなし、3件目をfailure notificationへ保存してpipelineを停止する。閾値と時間窓は設定可能とし、unknown、identity、validation、data-integrity failureの停止境界は変更しない。詳細と運用証拠は[TargetClosedExceptionによる収集全体停止を局所化する](changes/20260919_collection-target-closed-pause-recovery/README.md)を正本とする。

監視snapshotは `race-detail` taskのresource ID、Card/Result artifact状態、公式発走予定、task状態をread-only projectionとして返す。金曜18:00以降は直後の土日についてdiscoveryを分母にCard `Current` を評価し、0件は正常ではなくcoverage unknownとする。金曜21:00の未解消はCriticalとする。結果は公式発走予定+30分と開催日18:30に評価し、Result `Current` または公式な `Unavailable` を充足とする。findingは日付とcheckpointでfingerprintを固定し、監視処理はtask、pipeline、domain dataを変更しない。詳細は[週末レース情報の期限内収集を監視する](changes/20260919_collection-freshness-slo/README.md)を正本とする。
