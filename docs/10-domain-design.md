...
馬・調教師の公開プロフィールは、対象ID、公開識別子、引用元、取得日時と公開項目の辞書をイベントに保存し、JraSubjectProfileReadModelへ投影する。取得できた非空の値を更新し、欠落値は既存値を保持する。既存の氏名・性別・生年月日・馬主・所属の投影も更新する。現在のプロフィールと過去出走時点の馬主・調教師は分離する。

馬起点のレース取得では元の馬IDを維持する。日付・競馬場・Rで既存レースを照合し、未登録は作成する。再取得では確定済み状態を維持し、出走情報・結果・払戻8種類と追加メタデータを更新する。[決定と検証](changes/20260908_subject-refresh-horse-history/README.md)を参照。
