...
- 2026-09-12: レース一覧に残っていた独自button tabを標準`FluentTabs`へ移行し、収集管理とactive indicator・共通host class・tabpanel構造を統一した。標準presetに一致しない検索期間は「指定期間」と表示し、実際の条件と選択表示が食い違わないようにした。
- 2026-09-12: push後のGitHub Actionsで、CI固有のformat gateをローカル完了条件に含めていなかったため未整形コードを検出し、Unixのpath rootをseparator trim後の空文字で比較していたため通常の一時ディレクトリまでroot扱いするOS差分を検出した。全solutionへ`dotnet format`を適用し、root判定を`Directory.GetParent`へ変更して現在OSのfilesystem root拒否テストを追加した。以後はworkflowの正確なcommand、runner OS、remote run完了をpush closureとする規則を振り返りskillへ追加した。
...
- 2026-09-12: 追加レビューで、全体一時停止を理由必須の確認Dialogへ変更し、処理中Taskへの影響を事前表示した。Attempt詳細には処理時間、requested/final URL、HTTP status、page identification、エラーを表示し、Unexpected PageやResource不一致の調査情報を一画面で追跡可能にした。対象Componentテスト12件とResource detail Storeテスト1件が成功した。
...
- 2026-09-12: 過去Horse/TrainerがJRA公開検索・名簿で一意に同定できないケースをPermanentFailureではなくResourceNotFound（Unavailable）として記録するよう修正した。
