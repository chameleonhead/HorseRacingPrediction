...
| T2e | 統合CI相当と独立高risk review | Main + default reviewer | Lead + Review | 今回配備sliceのT2a-T2d | read-only | V1-V6、実行仕様§5全gate | verification.md: 現sliceの全体1198件/外部2件とreview閉鎖。未確定残存sliceに依存 | Dependent | Lead — integration/final acceptance。reviewerは独立反例 | T2e-A1 | unavailable; retries 0; corrections 0; reviews 3 |
