...
| T2b | Card/Result・振替の統合整合を確認/修正 | 解消task Main | Lead | T1a,T2a | S2とWorkerClient/CollectionPlatformEndpointExtensions/Store、CollectionCompletionTransportTests | V2/V5、identity/write反例、HTTP→DB→GET | transport欠落と初回複数stageのfacet重複を修正。HTTP4件red→green、全体検証/配備継続 | In progress | Lead — public contract/persistence/transport統合、短い配線と境界testは分割費用過大 | none | unavailable; retries 0; corrections 0; reviews 0 |
...
| T2e | 統合CI相当と独立高risk review | Main + default reviewer | Lead + Review | 今回配備sliceのT2a-T2d | read-only | V1-V6、実行仕様§5全gate | operations: 分類修正1213件/実サイト1成功。HTTP reviewのP1は再現→Store修正→4件成功→独立再review閉鎖。全体再検証中 | In progress | Lead — integration/final acceptance。reviewerは独立反例 | T2e-A1 | unavailable; retries 0; corrections 0; reviews 13 |
...
- `docs/26-collection-platform-design.md`: 完了通知のWorker/API/Store全境界でstage/facetと発走時刻証拠を転送する既存契約を明示。旧payload互換と過去証拠を推測復元しない境界、HTTP結合検証を正本へ補足。
