...
| --- | --- | --- |
| Code | Connected | 2つの収集運用workflowを削除し、契約テストと文書を更新した。mainへのmerge待ち。 |
| Verification | Connected | focused test、format、change-record validator、literal search成功。GitHub CI待ち。 |
| Deployment/operation | Not started | mainへのmerge後、workflow不存在と既存CI/CDの成功を確認する。 |
...
| --- | --- | --- | --- | --- |
| AC1 | mainに`collection-dlq-diagnostics.yml`と`collection-maintenance.yml`が存在せず、app/infraのCI/CD workflowは残る。 | T1 | file inventory、CI | Connected |
...
- **T1 — Main / High capability / In progress:** 2 workflowを削除し、契約テスト、正本文書、履歴supersession noteを更新する。Write scopeは対象workflow、`CollectionQueueCutoverContractTests.cs`、上記文書に限定する。AC1-AC3をfocused test、format、validator、literal search、GitHub CIで検証する。ローカルgateは成功し、main mergeとCIを残す。
...
- **Final review:** main mergeとCI確認後にAC、T1、completion summaryを照合する。
