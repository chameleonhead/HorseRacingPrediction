...
| Verification | In progress | ローカル回帰は成功。PR CIとreview完了が残る。 |
...
- current-generationかつ未配送、due、未予約のoutboxを持つtaskだけをdispatch candidateとする。
...
| --- | --- | --- | --- | --- |
| AC1 | 既配送Ready task、agingされたenvelope代表、group上限超過の反例で偽陽性と高priority除外が再発しない。 | T1 | focused tests、API regression、Release build | Verified |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| T1 | dispatch monitorとadmissionを修正し反例テストを追加する。AC1 | Main | High capability | Approval | monitoring、dispatcher、tests | focused/API/solution gates | commit `62eb745` | Verified | Lead — public contract and final acceptance | none | unavailable; retries 0; corrections 0; reviews 1 |
