...
Detail Page相当の独立ページとし、`RaceOpsPageHeader` に「運用 / データ補正」、説明、再読込Actionを置く。本文は次の順とする。
...
成功時は適用件数と既処理skip件数をMessageBarで示し、候補を再読込する。apply直前の再検証で競合・blockedになった場合は、変更されなかったこと、候補を再読込して理由を確認することを操作領域付近に表示する。技術的な例外本文はそのまま表示しない。
...
| AC9 | bUnit、repair API統合テスト、API全体テスト、実ブラウザーの通常・空・失敗・狭幅確認が成功する。 | Not started |
