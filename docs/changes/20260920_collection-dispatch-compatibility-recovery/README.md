...
| Verification | Complete | Store and component regressions, deployment, and production read-back passed. |
...
| AC3 | After deployment, the badge/API count equals the number of latest Running tasks with currently valid leases. | T2 | Production task-view count and task lease evidence. | Verified |
...
| T2 | Deploy once and verify the production count. | Main | Lead | T1 | deployment and read-only production diagnostics | API count and task lease evidence | Production count matches valid leases. | Verified |
...
- **Final review:** AC1-AC3 are Verified. The final diff changes only the task-view count predicate, its regression test, and this record; no error, retry, dispatcher, queue, or recovery path changed.

## Implementation and verification

- Commit `7aabc1b` changes `running` to count only latest Running tasks with `LeaseExpiresAt` later than the current JST time.
- The regression holds one active and one expired Running row, expects a count of one, and directly reads task and attempt rows afterward to prove the count request did not mutate them.
- Local verification passed: 93 `CollectionPlatformStoreTests`, 12 `CollectionAdministrationComponentTests`, solution build with zero warnings/errors, formatter verification, `git diff --check`, and the change-record validator.
- Production workflow `app-deploy` run `35485437639` completed successfully for commit `7aabc1b`.
- At 2026-09-20 12:11 JST, the production API reported processing count `0` and stored Running rows `0`, so the deployed count matched the set of tasks that could still be executing.
- There is no design deviation and no remaining scoped task.
