...
| T3 | Refactor the envelope-level Collector/session workflow to capture all race-detail tasks with one session, dispose once, and submit one envelope without polling. Covers AC1, AC3, AC4, AC6, AC8, AC10, AC11, AC20, AC21, AC27. | Main | Lead tier | T1, T2 | Collector collection-platform/session execution, race-detail workflow, focused tests | Microbatch ordering, partial capture, response-loss/retry, equivalence, heartbeat, cancellation, readiness counts, and mixed `RaceDay` definition tests | One-ingestion-call production path retaining session reuse | Proposed |
...
| T7 | Implement page-scoped leases, batched Jockey/Trainer directory resolution, sequential transient-page policy, descriptor reuse and profile ingestion. Covers AC16–AC19, AC21, AC25. | Main | Lead tier | T1,T2,T3 | Browser/session abstraction, JRA subject navigation, Collector subject handler, API application, focused tests | Directory-once, page-isolation, cleanup, fallback, no-op upsert and benchmark gates | One directory traversal per kind/group and one ingestion per subject envelope | Proposed |
| T8 | Implement and gate typed no-text navigation, single/page-kind readiness, browser request inventory/interception candidates, snapshot projection candidates, Collector-owned request compression, deadline-aware HTTP policy, and runtime experiment configuration against T2's frozen decode contract. Covers AC21–AC23, AC26, AC27. | Main | Lead tier | T1,T2,T3,T7 | Scraping browser/navigation/snapshot infrastructure and Collector HTTP/runtime configuration; no API decoder writes and no concurrent navigation | Barrier/resource/projection equality, Collector encoding, transport retry/idempotency tests and fixed-corpus runtime reports | Only measured candidates enabled; rejected candidates remain explicitly disabled | Proposed |
...
- **Design and task-split review** — 2026-09-15, reviewer: Main. Inputs: production AWS evidence, CodeGraph paths, semantic snapshot design records, current workflow/store/API code, delegated D1/D2/H1/H2/JT1/TAB1 and A1–A3 research, and independent D3/R2–R7 reviews. The original race-only design passed after two revision cycles covering completion ownership, response loss, envelope session reuse, cross-database idempotency, numeric budgets, partial capture and fencing. User then expanded scope to Horse history, cross-horse Race deduplication, Jockey/Trainer profiles, transient tabs, and further cost/latency opportunities. R2–R6 successively found stale scope and provenance bounds, parallel-rollout ambiguity, v1/v2 compatibility, compression/deadline safety, aggregate partial-success boundaries, and task ownership overlap. Follow-up aligns all subject/race definitions, freezes Race-frontier uniqueness and the 32-requester policy, unconditionally excludes production parallel navigation, separately verifies v1 definition-scoped and current/v2 mixed routing, and adds AC21–AC27 for browser waits/resources/snapshot projection, aggregate-grouped persistence, semantic no-op, telemetry/runtime cost, encoding safety, and deadline-aware retry. R7 independently returned `PASS`. D1/D2/D3/H1/H2/JT1/TAB1, A1–A3, R4, and R7 are `Verified` and made no tracked file changes. Rework: eight design-revision cycles, no implementation rework or escalation. Measured agent usage/cost was unavailable; elapsed investigation and revision count are the efficiency proxy.
