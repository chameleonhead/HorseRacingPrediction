...
freshnessの最小既存snapshot集計案（未承認・未実装）: 同一cutoffの`GetMonitoringSnapshotAsync` race artifact snapshotとdomain race read modelを用い、公式に期待されるrace集合を母集団`expected`とし、その内訳を永続化済み`Current`、`officialUnavailable`、それ以外の`unknown/missing`へ相互排他的に分類する。基本式は`expected = Current + officialUnavailable + unknown/missing`であり、集合重複、expected外の永続値、expected母数自体の不明を別途明示する。Resultの`Current`と`Unavailable`を合算せず、card/result別の件数、cutoff、source completeness、truncated状態を返す。既存read-only monitoring response/reportのbounded extensionまたは承認済みbounded queryとして親T3で扱い、今回のsnapshotから値を推定しない。

2026-09-25 C12 ScheduledRefresh metadata pre-implementation review — 親Mainのbounded read-only照合では、horse-profile `SubjectNotIdentified` の最新2件がcanonical horse API 200かつ永続locationありである一方、attempt 5のtask snapshotは`name`、`sourceIdentity`、`birthDate`が空で、`SubjectIdentification:MissingName`としてnavigation前に終了した。両resourceは初回Discoveryに明示attributes/locationを持ち、その後のScheduledRefreshから空snapshotが作られている。sourceでは`CollectionScheduleService`がScheduledRefreshをattributesなしで要求し、`ResolveTaskMetadataJsonAsync`はManualRefresh/Recovery以外を`SerializeTaskMetadata(attributes)`へ直送するため、既存resource metadataを保持しない。これは親Approved T2/T2fのshared metadata契約に属する確認済み局所欠陥であり、利用者の「既存のバグであればそのまま修正」の明示指示を承認根拠としてC12を`Resolved in design`、AC11を`Connected`、T10を`In progress`とする。

C12 frozen decision / scope: ScheduledRefreshでattributesが`null`の場合だけ、同resourceに永続化済みのmetadataを新task snapshotへコピーする。明示attributesは既存どおりresource/taskの完全な明示値として扱い、空dictionaryは意図的な空snapshot・resource更新を維持する。ManualRefresh/Recoveryのprior non-empty snapshot継承と指定key overlay、初回Discovery、既存task snapshotのimmutability、location、revision、lane/priority、同期・更新意味論、handlerのidentity validationを変更しない。canonical nameだけから`name`またはidentityを推定・結合しない。Trainer `SubjectResourceMissing`、freshness、public contract、schema/migration、DB補正、retry/Recovery、resume、workflow、production historyは範囲外である。

AC11: (a) name/sourceIdentityとexplicit locationを持つresourceのScheduledRefreshがnull attributesでも同metadataをlease/dispatchへ保持しhandlerのMissingName前提を解消する、(b)作成済みtask snapshotは後続resource属性変更の影響を受けない、(c)ScheduledRefreshの明示partial attributesはmergeせず明示値だけ、明示emptyは空を保持する、(d)ManualRefresh/Recovery、初回Discovery、locationは既存契約を維持する。反例は別resource混入、canonical read modelからの推定、明示emptyの暗黙復元、既存taskの後追い書換え。V17は実SQLite storeでscheduler相当のnull request、pending dispatch、acquire、resource更新後snapshot、明示partial/empty、ManualRefresh/Recovery/Discovery回帰を検証し、関連test、format、Release build、非External regression、既存CI/CDとdeployment safety gateへ接続する。production-shaped配備後も独立本番attempt観測前はAC11を`Verified`にせず`Connected`とする。

Agent task split — T10 ownerはMain、model tierはLead、write scopeは`CollectionPlatformStore.ResolveTaskMetadataJsonAsync`、同store専用test、本recordだけ。共有storeのmetadata意味論と既存fixtureが密結合した小sliceで、独立workerへのprompt/review/統合費用が実装を上回るため分割しない。completion evidenceはV17と既存CI/CD run、stateは`In progress`。同期・更新意味論、public contract、永続schemaまたは既存task書換えが必要と判明した場合は実装を止め、親Mainへ設計判断を返す。

2026-09-25 C12 local checkpoint — `ResolveTaskMetadataJsonAsync`はScheduledRefreshかつattributes `null`の場合だけ、既存resourceの永続`AttributesJson`を新task snapshotへコピーする。明示partial attributesは従来どおり完全置換、明示emptyは`{}`としてresource/taskへ保存され、次のnull ScheduledRefreshもemptyを維持する。既存task作成後にresource attributesを変更してもpending dispatchとleaseは元snapshotを保持し、explicit locationも維持した。handler反例は空snapshotが`SubjectIdentification:MissingName`になる既存testを保ち、保持済みname/sourceIdentity/locationを持つScheduledRefreshがprofile保存まで成功するtestを追加した。ManualRefresh/Recovery/Discoveryの既存契約、canonical推定、Trainer 404、freshness実装、schema/public contract/workflow/operationに差分はない。
