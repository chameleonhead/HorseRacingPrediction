...
## JRA cutover follow-up

The later approved direct cutover in `../20260908_jra-semantic-snapshot-cutover/README.md` made this semantic model the sole `IWebBrowser.GetPageSnapshotAsync` result, migrated all first-party consumers, and removed the former section-oriented compatibility snapshot. Earlier coexistence and impact-review notes above describe the pre-cutover state and are retained only as historical implementation context.
