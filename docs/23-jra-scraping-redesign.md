...
JRA向けNavigatorは、遷移・操作ごとにページ種別固有の必須DOMを1回待ち、その状態から待機を重ねずSemantic Snapshotを取得する。戻り値を使用しないtyped操作では本文全体や画像altを別途抽出しない。resource interceptionやpage-kind別Snapshot projectionは、全対象ページの正規化結果・diagnostic・identity証拠がcontrolと一致し、速度と失敗率のgateを満たす場合だけ有効化する。CSS、script、XHR/fetchや未知hostを推測で遮断しない。
...
SubjectProfilePageParserとJraNavigatorの馬検索・調教師名鑑・プロフィール・履歴ページ送りを実装する。公開ページの実リンクとフォームで遷移し、URLを推測生成しない。プロフィールは全dt/dd、履歴は行数制限のない全テーブルを取得する。氏名・生年月日・公開識別子を照合し、曖昧な対象は更新しない。馬は実際の詳細リンク、調教師は氏名と生年月日の複合識別子を保持する。レース結果は既存解析器で日付・競馬場・Rと対象馬の出走を確認する。[決定と検証](changes/20260908_subject-refresh-horse-history/README.md)を参照。
