...
- 2026-09-18提案: handlerが型付き契約で対象単位に隔離可能と明示した構造エラーは、失敗試行と要対応通知を保存して当該taskだけを終端し、無関係な配送と新規リースは継続する。impact欠落と未分類の恒久障害は従来どおり収集全体を安全停止する。設計と承認状態は[構造エラー隔離](changes/20260918_isolate-structural-collection-failures/README.md)を参照し、承認・実装までは現行動作を維持する。
