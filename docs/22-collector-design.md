...
2026-09-18、本番で高速化後の一括レース保存が競走馬のJRA identityを落とし、RaceEntryと子Taskに異なるIDを作る回帰を確認した。騎手・調教師にも経路別の名称正規化差、馬主には別ID体系と未登録Taskの危険がある。修正は個別Taskの削除や名称だけの統合ではなく、bulkの効率を維持した共通解決結果の再利用と、強い識別子・同一RaceEntry・Owner aliasを根拠にした過去ジョブのpreview/applyで行う。設定画面では変更なしのpreview、分類・根拠・予定操作を確認後、安全対象だけを選択してmigrationできるようにする。曖昧、証拠不足、同名別主体は自動統合しない。設計、主体別fallback表、画面契約、再検証条件は[主体ジョブのcanonical ID整合と過去ジョブフォールバック](changes/20260918_canonical-subject-job-fallback/README.md)を正とし、承認前は本番データを変更しない。
...
2026-09-19提案: RaceCardのHorse identityを欠く旧task、またはRaceResultだけを取得する過去レースで同名候補が複数になる場合、まず起点レース結果の該当馬番行にある検証済みJRA Horse linkをidentityとして伝播する。linkを利用できない場合は、Race由来であること、canonical `requestedByRaceId`、`EffectiveDate` の整合を検証したうえで、候補プロフィールの全公開出走履歴に起点レースの日付・場・番号が完全一致する1頭だけを選択できるようにする。生年月日がレース後、抹消年月日がレース前という時系列矛盾は候補排除と診断に使うが、時期や結果ページの性齢だけで残り候補を採用しない。過去結果・履歴が公開範囲外なら安全停止し、現在の同名馬へ置き換えない。保存済みJRA identityと生年月日の既存優先順位、同名別馬をmergeしない規則は維持する。設計と反例は[レース根拠で同名競走馬を安全に解決する](changes/20260919_race-evidence-horse-resolution/README.md)を正本とし、承認前は現行動作を変更しない。
...
2026-09-19再提案: canonical Raceとactive `race-detail` taskは各1件のまま維持し、Card/Resultの取得状態をRace resource配下の独立facetとして正本化する。Taskはfacetを前進させるpersistent controllerとし、Attemptは実行stageごとのoutcomeをappend-onlyで残す。公式StartTimeは更新可能なresource evidenceに保存し、発走前Result遷移を禁止する。同日RaceはRaceDay Envelopeと共有JRA session、現在ページ短絡、bulk writeで高速化する。旧履歴は書き換えず、既存エラージョブの自動復旧は行わない。設計は[Raceリソース中心の取得状態機械](changes/20260919_race-detail-phase-recovery/README.md)を正とし、承認前は現行動作を変更しない。

5日は出馬表を優先して馬主等を補完するための期間であり、JRAの掲載保証ではない。期間内でも出馬表の開催選択が`OutOfDisplayedRange`になった過去レースは、構造エラーや未来の未公開へ丸めず、既存の結果導線へ切り替える。discovery、race-detail、過去日の結果導線に限定した境界修正案は[Dynamic race source fallback](changes/20260917_dynamic-race-source-fallback/README.md)を正本とし、承認前は現行動作を変更しない。
