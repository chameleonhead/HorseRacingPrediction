...
2026-09-19提案: RaceCardのHorse identityを欠く旧taskで同名候補が複数になる場合、Race由来であること、canonical `requestedByRaceId`、`EffectiveDate` の整合を検証したうえで、候補プロフィールの公開出走履歴に起点レースの日付・場・番号が完全一致する1頭だけを選択できるようにする。生年月日がレース後、抹消年月日がレース前という時系列矛盾は候補排除と診断に使うが、時期だけで残り候補を採用しない。保存済みJRA identityと生年月日の既存優先順位、同定不能時の安全停止、同名別馬をmergeしない規則は維持する。設計と反例は[レース根拠で同名競走馬を安全に解決する](changes/20260919_race-evidence-horse-resolution/README.md)を正本とし、承認前は現行動作を変更しない。
