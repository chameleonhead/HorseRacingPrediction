...
> 2026-09-27 出馬表取消契約: 実出馬表では取消馬の専用馬番セルが「取消」に置換され、公式Horse linkと過去成績の別馬番が同じ行に残る。専用セルの「取消」「除外」を発走前の出走状態として保持し、番号を行順・隣接行・過去成績から補完しない。取消の番号nullを通常馬の未確定待ちと区別し、全行の保存・読出しとActiveのみの予想を分離する。実サイトの観測範囲は§2.3、検証・配備状態は[出馬表取消対応](changes/20260927_race-card-cancellation/README.md)を参照。
...
> 同一SQLite volumeのRace/主体ロックで全関連writerと予想用履歴readerを排他する。補正未整合gateはprocess再起動後も維持し、sidecarとDB event/projectionが食い違うbackup復元も利用不可にする。旧binaryへの単純rollbackは禁止。補正直前に対象ロック内でEvent/Collection DBのSQLite online backupと対象sidecar、manifest/hashをpackage化し、読出し・整合検査失敗なら補正しない。他Raceが進む複数DB全体の同一時点は保証せず、復元は独立環境で対象整合を検査する。本番restoreは別途承認。
...
> 2026-09-26 承認済み契約: 天候・馬場状態は対象Raceの専用欄だけから取得し、本文の馬主・馬名・勝馬情報を検索しない。未知値、重複、通常結果の必須欠落は保存前に拒否する。[馬場状態誤認と停止復旧設計](changes/20260919_race-entry-owner-enrichment/decisions/20260926-track-condition-recovery.md)を参照。Resultの勝馬紹介にowner文字列が存在する事実と、全出走馬のowner取得元として採用することは別であり、下記Card限定契約は変更しない。
...
2026-09-27、中山1Rの公式出馬表で確認したサイト知識（実装完了を意味しない）:

- 取消馬も出馬表の同じ表に1頭分の行として残る。今回の行では、専用馬番セル `td.num` の `span.cap` に数値ではなく `取消` が表示されていた。
- その行の馬名・公式Horse link・枠・馬主・調教師・騎手等は残る。取得できる情報は取消状態とともに記録できる。取消だから行全体を取得対象外にしない。
- 同じ行の過去成績には別レースの馬番がある。現在の馬番が表示されない場合、過去成績・行順・前後の番号から補完してはならない。
- この観測で確認した表示は `取消`。`除外` や別のHTML構造まで実サイト確認済みとは扱わず、観測事実と対応予定を分ける。
- 取得URL、対象馬、再現方法は[取消表示の確認記録](changes/20260927_race-card-cancellation/evidence.md)を参照する。

利用者確認済みの記録方針: **出馬表の記録対象と予想対象は別**とする。取消馬も通常馬と同様に出馬表の一員として保存・読出し対象に残し、予想候補を作る段階でのみ除外する。馬名、馬主等の取得済み情報や安定EntryIdを削除しない。番号が表示されないことと取消であることを区別できる状態を持たせる。対応の実装・検証状況は[変更記録](changes/20260927_race-card-cancellation/README.md)を正とする。

## 3. 画面と取得情報

### 開催中止の証拠

月間カレンダーは収集候補であり、ニュース中の競馬場名や一般的な中止注意文を開催状態の確定証拠にしない。
候補のレース一覧が結果fallbackを含めて表示範囲外の場合、公式日別番組の表示日・競馬場付き開催caption・同table内の全開催中止通知を照合する。
明示中止だけを対象外とし、対象日/course・開催番号/日番号・公式URLをattempt診断へ保存する。同日他場は継続し、代替日は公式日程上の独立日として収集する。
日付推測・既存Race ID書換え・他場への中止伝播は禁止。未知文言、誤ページ、通信障害は取消確定でも成功でもない。
全候補中止はNotApplicable、他場成功と中止混在は取得件数と除外件数を区別する。
2026-09-24確認のURL・HTML境界・制限・反例は [JRAサイト確認記録](changes/20260924_collection-error-closure/evidence/jra-site-observations.md) を参照。
...
- Collection taskの成功とデータ完全性を分離する。Cardが存在しないRaceResult収集は成功し得るが、
  ownerが取得済みであるとは扱わない。
- 保存済みCardが旧抽出版数のまま探索期間外になった場合もResult収集を妨げない。Card facetは`Unavailable / OfficialRaceCardOutsideLookupPeriod`、要求版数は新しい値、適用版数と最終保存時刻は旧値を保持して、更新不能な部分成功を明示する。
- この不足表示は次の版数更新でも更新する。旧Card/Resultからの統合移行では、日時を伴う収集済み証拠を統合版数1のfacetとして引き継ぐ。既存の統合状態・facetの新しい版数や保存時刻を古い情報で上書きせず、日時のない旧Currentだけから保存済みとは推定しない。既に失われた移行前証拠を復元する処理ではない。
...
- 日別番組 `/keiba/calendar{year}/{year}/{month}/{MMdd}.html` は複数開催の表を含む確認用ページであり、単一競馬場のRaceListではない。中止確認後に同じbrowser sessionへ残っていても通常一覧として解析せず、後続対象は通常の一覧遷移へ進む。未知のJRADB一覧の解析失敗を握り潰してはならない。
