# JraSiteDataCollector — 設計仕様書

> このコンポーネントは Collector の実装詳細である。全体構成・責務境界は [collector-design.md](collector-design.md) を参照。
>
> 旧名 `JraTaskAgent`。LLM を使わない機械的なブラウザ操作コンポーネントであり AI Agent ではないことを明確にするため、`JraSiteDataCollector` に改名し、`HorseRacingPrediction.Agents` プロジェクトから `HorseRacingPrediction.Scraping`（`JraNavigation` フォルダ）へ移動した。

## 概要

`JraSiteDataCollector` は JRA 公式サイトを「人間のように」操作する、状態保持型のナビゲーションコンポーネントです。
単なるスクレイパーの集合ではなく、**現在ページを起点にして最短経路で目的の情報へ到達し、必要があれば戻る**という、人間のブラウジング操作をそのまま抽象化します。LLM は使用せず、すべて機械的なルールベースのロジックで遷移を判断します。

Playwright セッションを自身で保持し、`IAsyncDisposable` で安全に終了できます。
...
新しい設計:
- コレクターが **セッション＋コンテキスト** を持つ
- 呼び出し側は「何を取りたいか」だけを依頼する
- コレクターが「どこから辿るか」「何をクリックするか」「戻るか」をルールベースで判断する
...
### JraSiteDataCollector（公開 API）

```csharp
await using var collector = await JraSiteDataCollector.CreateAsync();

// レース情報
await collector.RequestRaceCardAsync(date, racecourse, raceNumber);
await collector.RequestOddsAsync(date, racecourse, raceNumber);
await collector.RequestRaceResultAsync(date, racecourse, raceNumber);

// エンティティプロフィール（現在ページからリンクを辿る）
await collector.RequestHorseProfileAsync("デアトゥバトル");
await collector.RequestJockeyProfileAsync("松若 風馬");
await collector.RequestTrainerProfileAsync("水野 貴広");

// 低レベル操作
await collector.ExtractCurrentPageAsync();
await collector.FollowAsync("オッズ");
await collector.BackAsync();
```
...
```
src/HorseRacingPrediction.Scraping/JraNavigation/
...
├── JraRaceCardUrlDiscoverer.cs  # 出馬表 URL のブラウザ操作による発見
├── JraRaceResultUrlDiscoverer.cs # 成績 URL のブラウザ操作による発見
...
| v1.1 | 2026-07-04 | `JraTaskAgent` → `JraSiteDataCollector` に改名し、`HorseRacingPrediction.Agents` から `HorseRacingPrediction.Scraping` へ移動（AI Agent ではないことの明確化） |
