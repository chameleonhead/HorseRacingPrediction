...
- **原則: 1ファイル1クラス**
- エージェント、Plugin、Tool ラッパー、Options、DTO、補助サービスは、ネスト型などの例外を除き、1型ごとに独立ファイルへ配置する
- 小規模な関連型でも同一ファイルへまとめず、責務単位で分割する
...
- 関連する `Options`, `Result`, `Context`, `State` を追加する場合も別ファイルに分割する
...
- 統合テスト: `HorseRacingPrediction.Agents.Tests` など Agents 側の関連テストで実際のブラウザ + LLM を用いて E2E 検証。Collector は AI エージェントを持たない
