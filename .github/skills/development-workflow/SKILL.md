...
│   ├── {EventName}.cs              # イベント（1イベント1ファイル）
│   └── ...                         # 各イベントを個別ファイルに
```

- `EventFlow.Aggregates.AggregateRoot<TAggregate, TIdentity>` を継承
- 状態の不正遷移は `InvalidOperationException` をスロー
- イベントは `AggregateEvent<TAggregate, TIdentity>` を継承するクラス
- **各クラス・レコード・enum は独立したファイルに配置すること**
...
- ReadModel の関連型（Snapshot、Entry、enum）も個別ファイルに配置
...
- **Contracts**: リクエスト・レスポンス DTO は `Contracts/` 配下に1クラス1ファイルで配置
