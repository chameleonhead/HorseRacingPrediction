...
public sealed partial class CollectionExecutionServiceIntegrationTests
{
    ...
    public async Task HorseHistory_PagesChildrenPartialFailureAndRetryArePersisted(bool single)
    {
        ...
        var sessions = new FakeJraSessionFactory
        {
            ...
            {
                SubjectFactory = _ => { discoveries++; return page1; },
                NextHistoryFactory = p => p == page1 ? page2 : null,
                HistoryResultFactory = (horse, race) =>
                {
                    attempts.Add(race.RaceName); if (fail && race.RaceName == "レースA") throw new InvalidOperationException("取得失敗");
