...
public sealed class JraNavigatorTests
{
    ...
    [TestMethod]
    public async Task ToRaceCardAsync_RaceListCardUrl_NavigatesWithoutRewritingValidJraUrl()
    {
        const string raceListUrl = "https://www.jra.go.jp/keiba/sample/racelist/";
        const string cardUrl = "https://www.jra.go.jp/JRADB/accessD.html?CNAME=pw01dde0106202604011120260905/2F";
        var browser = new FakeWebBrowser();
        browser.SetSnapshot(CalendarUrl, BuildCalendarSnapshot(CalendarUrl, []));
        browser.SetClickDestination("出馬表", MeetingSelectionUrl);
        browser.SetSnapshot(MeetingSelectionUrl,
            BuildMeetingSelectionSnapshot(MeetingSelectionUrl, "9月5日 4回中山1日"));
        browser.SetClickDestination("4回中山1日", raceListUrl);
        browser.SetSnapshot(raceListUrl, BuildRaceListSnapshotWithCardLink(raceListUrl, cardUrl));
        ...
        Assert.AreEqual(JraPageKind.RaceCard, page.Kind);
        CollectionAssert.Contains(browser.NavigatedUrls, cardUrl);
    }
    ...
    [TestMethod]
    public async Task ToRaceResultAsync_CurrentPastPeriod_OutOfDisplayedRange_FallsBackToHistoricalRoute()
    {
        const string searchUrl = "https://www.jra.go.jp/keiba/sample/search/current-past/";
        const string searchResultUrl = "https://www.jra.go.jp/keiba/sample/search/current-past/result/";
        const string raceResultUrl = "https://www.jra.go.jp/keiba/sample/search/current-past/result/11/";
        ...
        browser.SetSnapshot(
            ResultSelectionUrl,
            BuildMeetingSelectionSnapshot(ResultSelectionUrl, "9月6日 4回中山1日"));
        browser.SetClickDestination("過去レース結果検索", searchUrl);
        browser.SetSubmitDestination(searchResultUrl);
        browser.SetSnapshot(
            searchResultUrl,
        ...
        Assert.AreEqual(JraPageKind.RaceResult, page.Kind);
        CollectionAssert.Contains(browser.NavigatedUrls, raceResultUrl);
        Assert.IsNotEmpty(browser.SelectOptionCalls);
    }

    [TestMethod]
    public async Task ToRaceResultAsync_HistoricalDirectResultForAnotherRace_NavigatesToRequestedRace()
    {
        const string searchUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct/";
        const string searchResultUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct/result/";
        const string directResultUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct/result/11/";
        const string requestedResultUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct/result/1/";
        ...
        browser.SetSnapshot(
            searchResultUrl,
            BuildMeetingSelectionSnapshot(searchResultUrl, "1月5日 1回中山2日"));
        browser.SetClickDestination("1回中山2日", directResultUrl);
        browser.SetSnapshot(directResultUrl, BuildRaceResultSnapshot(directResultUrl, "11R", "2026年1月5日 中山"));
        browser.SetLinks(directResultUrl, [new TestPageLink(requestedResultUrl, "1レース結果")]);
        browser.SetSnapshot(requestedResultUrl, BuildRaceResultSnapshot(requestedResultUrl, "1R", "2026年1月5日 中山"));

        var navigator = new JraNavigator(
            browser,
            CreateReader(browser),
            logger: null,
            today: () => new DateOnly(2026, 9, 17));

        var page = await navigator.ToRaceResultAsync(new RaceId(date, RaceCourse.Nakayama, 1));

        Assert.AreEqual(new RaceId(date, RaceCourse.Nakayama, 1), ((JraRaceResultPage)page).RaceId);
        CollectionAssert.Contains(browser.NavigatedUrls, requestedResultUrl);
    }

    [TestMethod]
    public async Task ToRaceResultAsync_HistoricalDirectResultForAnotherRaceWithoutRequestedLink_ThrowsIdentityMismatch()
    {
        const string searchUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct-no-link/";
        const string searchResultUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct-no-link/result/";
        const string directResultUrl = "https://www.jra.go.jp/keiba/sample/search/wrong-direct-no-link/result/11/";
        var date = new DateOnly(2026, 1, 5);
        ...
        var exception = await Assert.ThrowsExactlyAsync<JraRaceIdentityMismatchException>(() =>
            navigator.ToRaceResultAsync(new RaceId(date, RaceCourse.Nakayama, 1)));

        StringAssert.Contains(exception.Message, new RaceId(date, RaceCourse.Nakayama, 1).ToString());
        Assert.AreEqual(new RaceId(date, RaceCourse.Nakayama, 11).ToString(), exception.RawValue);
    }

    [TestMethod]
    public async Task ToRaceResultListAsync_CurrentPastPeriod_OutOfDisplayedRange_FallsBackToHistoricalRoute()
    {
        const string searchUrl = "https://www.jra.go.jp/keiba/sample/search/current-past-list/";
        const string searchResultUrl = "https://www.jra.go.jp/keiba/sample/search/current-past-list/result/";
        const string raceListUrl = "https://www.jra.go.jp/keiba/sample/search/current-past-list/result/list/";
        ...
        Assert.AreEqual(JraPageKind.RaceList, page.Kind);
        CollectionAssert.Contains(browser.NavigatedUrls, raceListUrl);
        Assert.IsNotEmpty(browser.SelectOptionCalls);
    }

    [TestMethod]
    public async Task ToRaceResultAsync_CurrentDay_OutOfDisplayedRange_DoesNotFallback()
    {
        var browser = new FakeWebBrowser();
        browser.SetCurrentUrl(KeibaTopUrl);
        browser.SetClickDestination("レース結果", ResultSelectionUrl);
        browser.SetSnapshot(
            ResultSelectionUrl,
            BuildMeetingSelectionSnapshot(ResultSelectionUrl, string.Empty));
        var navigator = new JraNavigator(
            browser,
            CreateReader(browser),
            logger: null,
        ...
        var exception = await Assert.ThrowsExactlyAsync<JraNavigationException>(() =>
            navigator.ToRaceResultAsync(
                new RaceId(new DateOnly(2026, 9, 5), RaceCourse.Nakayama, 11)));

        Assert.AreEqual(JraNavigationFailureReason.OutOfDisplayedRange, exception.Reason);
        Assert.IsEmpty(browser.SelectOptionCalls);
    }

    [TestMethod]
    public async Task ToRaceResultListAsync_CurrentDay_OutOfDisplayedRange_DoesNotFallback()
    {
        var browser = new FakeWebBrowser();
        browser.SetCurrentUrl(KeibaTopUrl);
        browser.SetClickDestination("レース結果", ResultSelectionUrl);
        ...
        var exception = await Assert.ThrowsExactlyAsync<JraNavigationException>(() =>
            navigator.ToRaceResultListAsync(
                new DateOnly(2026, 9, 5),
                RaceCourse.Nakayama));
