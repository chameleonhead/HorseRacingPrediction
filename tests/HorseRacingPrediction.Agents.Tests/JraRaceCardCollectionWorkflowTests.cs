...
public class JraRaceCardCollectionWorkflowTests
{
    ...
    public async Task DiscoverUrlsAsync_FiltersUrlsByRequestedDate()
    {
        ...
        Assert.HasCount(1, result, "指定日以外の URL はフィルタされること");
        Assert.AreEqual(new DateOnly(2025, 4, 20), result[0].RaceDate);
    }
    ...
    [TestMethod]
    public async Task ScrapeAllAsync_WithValidUrls_ReturnsScrapedData()
    {
        _fakeWebBrowser.Snapshot = new PageSnapshot(
            ...
            [
                new PageTableSnapshot(
                    ["枠番", "馬番", "馬名", "騎手", "斤量"],
                    [
                        ["1", "1", "イクイノックス", "川田将雅", "58.0"],
                        ["2", "2", "リバティアイランド", "戸崎圭太", "56.0"],
                    ])
            ]);

        var urls = new[]
        {
            JraRaceCardUrl.ParseFromUrl(
    ...
    [TestMethod]
    public async Task SaveAllAsync_WithValidData_PublishesCommands()
    {
        var url = JraRaceCardUrl.ParseFromUrl(
            "https://www.jra.go.jp/JRADB/accessD.html?CNAME=pw01sde0203_20251026051101&sub=",
            "東京");

        var data = new JraRaceCardData(
            Url: url.Url,
            RaceName: "天皇賞（秋）",
            Racecourse: "東京",
            ...
            Entries:
            [
                new JraRaceEntryData(1, 1, "イクイノックス", "川田将雅", 58.0m, "牡4", 520m, 0m, "木村哲也", null),
            ]);
        ...
        CollectionAssert.Contains(_fakeCommandBus.PublishedCommandNames, "RegisterEntryCommand",
            "RegisterEntryCommand が発行されること");
    }

    [TestMethod]
    public async Task SaveAllAsync_MissingRaceDate_SkipsAndReportsError()
    {
        var url = new JraRaceCardUrl("https://www.jra.go.jp/test", "東京", null, null, null);
        var data = new JraRaceCardData(
            Url: url.Url,
            RaceName: "テストレース",
            Racecourse: "東京",
        ...
        Assert.IsEmpty(savedIds, "保存されないこと");
        Assert.HasCount(1, errors, "エラーが1件報告されること");
    }

    [TestMethod]
    public async Task SaveAllAsync_MissingRaceNumber_SkipsAndReportsError()
    {
        var url = new JraRaceCardUrl("https://www.jra.go.jp/test", "東京", null, new DateOnly(2025, 4, 20), null);
        var data = new JraRaceCardData(
            Url: url.Url,
            RaceName: "テストレース",
            Racecourse: "東京",
            RaceDate: new DateOnly(2025, 4, 20),
            RaceNumber: null, // ← レース番号なし
            CourseType: null,
            Distance: null,
            Grade: null,
            Entries: []);
    ...
    [TestMethod]
    public async Task SaveAllAsync_UsesUrlRaceDateAsFallback()
    {
        // URL の CNAME から日付・競馬場を取得できる場合
        var url = JraRaceCardUrl.ParseFromUrl(
            "https://www.jra.go.jp/JRADB/accessD.html?CNAME=pw01sde0203_20251026051101&sub=",
            "東京");

        // スクレイプ結果には日付・競馬場が入っていない
        var data = new JraRaceCardData(
            Url: url.Url,
            RaceName: "天皇賞（秋）",
            Racecourse: null,   // ← 未取得
        ...
        Assert.HasCount(1, savedIds, "URL のメタ情報をフォールバックとして保存できること");
    ...
    public async Task CollectAsync_EndToEnd_ReturnsPopulatedResult()
    {
        ...
        Assert.AreEqual(new DateOnly(2025, 10, 26), result.WeekendDate);
        Assert.HasCount(1, result.DiscoveredUrls, "URL が1件発見されること");
        Assert.HasCount(1, result.ScrapedCards, "出馬表が1件スクレイプされること");
