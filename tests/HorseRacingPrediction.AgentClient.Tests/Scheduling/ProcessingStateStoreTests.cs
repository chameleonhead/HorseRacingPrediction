...
public sealed class ProcessingStateStoreTests
{
    ...
    [TestMethod]
    public async Task MarkTextInsightRecordedAsync_PersistsAcrossRestart()
    {
        const string insightKey = "race-1:20260512:abcdef";

        var sut = CreateStore(predictionLeaseMinutes: 5);
        await sut.MarkTextInsightRecordedAsync(insightKey);

        var restarted = CreateStore(predictionLeaseMinutes: 5);
        var recorded = await restarted.IsTextInsightRecordedAsync(insightKey);
