...
public sealed class HistoricalDataRequestPlanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (await HasMissingHistoryAsync(entry, cancellationToken).ConfigureAwait(false))
            {
                return await EnsureRaceResultRequestsAsync(context, raceId, now, cancellationToken).ConfigureAwait(false);
            }
        }
    ...
    private async Task<bool> HasMissingHistoryAsync(
        RacePredictionContextEntry entry,
        CancellationToken cancellationToken)
    {
        var history = await _raceQueryService.GetHorseRaceHistoryAsync(entry.HorseId, cancellationToken).ConfigureAwait(false);
        ...
        return jockeyHistory is null || jockeyHistory.Entries.Count == 0;
    }
