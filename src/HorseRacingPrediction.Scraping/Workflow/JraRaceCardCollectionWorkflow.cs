...
///   <item><see cref="ScrapeAllAsync"/> — 各 URL を Playwright で決定的にスクレイプする</item>
///   <item><see cref="SaveAllAsync"/> — スクレイプ結果を EventFlow コマンド経由で保存する</item>
...
public sealed class JraRaceCardCollectionWorkflow
{
    ...
    private readonly JraRaceCardUrlDiscoverer _discoverer;
    ...
    internal JraRaceCardCollectionWorkflow(
        JraRaceCardUrlDiscoverer discoverer,
    ...
    public JraRaceCardCollectionWorkflow(
        ...
        JraRaceCardScraper scraper,
        DataCollectionWriteTools writeTools)
        : this(new JraRaceCardUrlDiscoverer(browser), scraper, writeTools)
    {
    }

    /// <summary>
    /// 指定した週末の出馬表 URL 一覧をブラウザ操作で発見して返す。
    /// </summary>
    public Task<IReadOnlyList<JraRaceCardUrl>> DiscoverUrlsAsync(
        DateOnly weekendDate,
        CancellationToken cancellationToken = default)
        => _discoverer.DiscoverUrlsAsync(weekendDate, cancellationToken);
