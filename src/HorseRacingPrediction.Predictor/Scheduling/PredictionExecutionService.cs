...
public sealed class PredictionExecutionService : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogInformation("PredictionExecutionService は設定で無効化されています。");
            return;
        }

        _logger.LogInformation("PredictionExecutionService (API-only) を開始しました。");
    ...
    private async Task RunOneCycleAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogDebug("[予想-API_ONLY] 処理対象レースはありません。");
            return;
        }
        ...
        foreach (var raceId in candidates)
        {
            try
            {
                if (_options.BlockPredictionWhileHistoricalRequestsPending)
                {
                    ...
                    if (summary.TotalPendingRequests > 0)
                    {
                        _logger.LogInformation(
                            "[予想-API_ONLY] 過去データ補完待ちのため再投入します。RaceId={RaceId} HorseRequests={HorseRequests} JockeyRequests={JockeyRequests} RaceResultRequests={RaceResultRequests}",
                            raceId,
                ...
                _logger.LogInformation(
                    "[予想-API_ONLY] 完了: RaceId={RaceId} TicketId={TicketId} SummaryLength={SummaryLength}",
                    raceId,
