...
public sealed class EventFlowDataCollectionWriteService : IDataCollectionWriteService
{
    ...
    public async Task<string> UpsertRaceAsync(string raceDate, string racecourseCode, int raceNumber, string raceName, int? entryCount, string? gradeCode, string? surfaceCode, int? distanceMeters, string? directionCode, CancellationToken cancellationToken = default)
    {
        var parsedRaceDate = DateOnly.Parse(raceDate, CultureInfo.InvariantCulture);
        var raceId = new RaceId(DeterministicIdGenerator.BuildRaceId(parsedRaceDate, racecourseCode, raceNumber));
        var existing = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<RacePredictionContextReadModel>(raceId.Value), cancellationToken);

        if (existing is null || string.IsNullOrEmpty(existing.RaceId))
        {
            var createCommand = new CreateRaceCommand(raceId, parsedRaceDate, racecourseCode, raceNumber, raceName, gradeCode: gradeCode, surfaceCode: surfaceCode, distanceMeters: distanceMeters, directionCode: directionCode);
            await PublishOrThrowAsync(createCommand, $"レース作成に失敗しました: {raceId.Value}", cancellationToken);
        }
        else
        {
            var correctCommand = new CorrectRaceDataCommand(raceId, raceName: raceName, racecourseCode: racecourseCode, raceNumber: raceNumber, gradeCode: gradeCode, surfaceCode: surfaceCode, distanceMeters: distanceMeters, directionCode: directionCode, reason: "Collected by data collection agent");
        ...
        if (entryCount is > 0 && (existing is null || existing.Status == RaceStatus.Draft))
        {
            ...
            await PublishOrThrowAsync(publishCardCommand, $"出馬表公開に失敗しました: {raceId.Value}", cancellationToken);
        }

        return raceId.Value;
    }

    public async Task<string> UpsertHorseAsync(string registeredName, string? normalizedName, string? sexCode, string? birthDate, CancellationToken cancellationToken = default)
    {
        ...
        var existing = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<HorseReadModel>(horseId.Value), cancellationToken);
    ...
    public async Task<string> UpsertJockeyAsync(string displayName, string? normalizedName, string? affiliationCode, CancellationToken cancellationToken = default)
    {
        ...
        var existing = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<JockeyReadModel>(jockeyId.Value), cancellationToken);
    ...
    public async Task<string> UpsertTrainerAsync(string displayName, string? normalizedName, string? affiliationCode, CancellationToken cancellationToken = default)
    {
        var normalized = DeterministicIdGenerator.NormalizeDisplayName(normalizedName ?? displayName);
        var trainerId = new TrainerId(DeterministicIdGenerator.BuildEntityId("trainer", normalized));
        var existing = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<TrainerReadModel>(trainerId.Value), cancellationToken);
    ...
    public async Task<string> UpsertRaceEntryAsync(string raceId, int horseNumber, string horseName, string? jockeyName, string? trainerName, int? gateNumber, decimal? assignedWeight, string? sexCode, int? age, decimal? declaredWeight, decimal? declaredWeightDiff, CancellationToken cancellationToken = default)
    {
        var race = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<RacePredictionContextReadModel>(raceId), cancellationToken);
        ...
        var horseId = await UpsertHorseAsync(horseName, null, sexCode, null, cancellationToken);
        var jockeyId = string.IsNullOrWhiteSpace(jockeyName) ? null : await UpsertJockeyAsync(jockeyName, null, null, cancellationToken);
        var trainerId = string.IsNullOrWhiteSpace(trainerName) ? null : await UpsertTrainerAsync(trainerName, null, null, cancellationToken);

        var command = new RegisterEntryCommand(new RaceId(raceId), DeterministicIdGenerator.BuildRaceEntryId(raceId, horseNumber), horseId, horseNumber, jockeyId, trainerId, gateNumber, assignedWeight, sexCode, age, declaredWeight, declaredWeightDiff);
    ...
    public async Task<string> DeclareRaceResultAsync(string raceId, string winningHorseName, string? declaredAt, string? winningHorseId, CancellationToken cancellationToken = default)
    {
        var race = await _queryProcessor.ProcessAsync(new ReadModelByIdQuery<RacePredictionContextReadModel>(raceId), cancellationToken);
        if (race is null || string.IsNullOrEmpty(race.RaceId))
            throw new InvalidOperationException($"レースが存在しません: {raceId}");
        ...
        var parsedAt = declaredAt is not null ? DateTimeOffset.Parse(declaredAt, CultureInfo.InvariantCulture) : DateTimeOffset.UtcNow;
    ...
    public async Task<string> DeclareRaceEntryResultAsync(string raceId, int horseNumber, int? finishPosition, string? officialTime, string? marginText, string? lastThreeFurlongTime, string? abnormalResultCode, decimal? prizeMoney, CancellationToken cancellationToken = default)
    {
        var entryId = DeterministicIdGenerator.BuildRaceEntryId(raceId, horseNumber);
        var command = new DeclareEntryResultCommand(new RaceId(raceId), entryId, finishPosition, officialTime, marginText, lastThreeFurlongTime, abnormalResultCode, prizeMoney);
    ...
    public async Task<string> DeclareRacePayoutsAsync(string raceId, string? winPayoutsJson, string? placePayoutsJson, string? quinellaPayoutsJson, string? exactaPayoutsJson, string? trifectaPayoutsJson, CancellationToken cancellationToken = default)
    {
        var command = new DeclarePayoutResultCommand(new RaceId(raceId), DateTimeOffset.UtcNow, ParsePayouts(winPayoutsJson), ParsePayouts(placePayoutsJson), ParsePayouts(quinellaPayoutsJson), ParsePayouts(exactaPayoutsJson), ParsePayouts(trifectaPayoutsJson));
        await PublishOrThrowAsync(command, $"払い戻し記録に失敗しました: {raceId}", cancellationToken);
        return $"レース {raceId} の払い戻しを記録しました。";
    }

    private async Task PublishOrThrowAsync<TAggregate, TIdentity>(Command<TAggregate, TIdentity> command, string errorMessage, CancellationToken cancellationToken)
