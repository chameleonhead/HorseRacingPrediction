...
public sealed partial class ProcessingStateStore
{
        ...
        string actor, DateTimeOffset now, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var job = new ProcessingJobEntity { JobId=BuildJobId(jobType,key), JobType=jobType, DeduplicationKey=key,
                Payload=AgentJobPayloadSerializer.Serialize(payload), Status=AgentJobStatus.Ready, Priority=100,
                FirstQueuedAt=now, AvailableAt=now, CreatedAt=now, UpdatedAt=now };
            db.Jobs.Add(job);
            db.JobOperationAudits.Add(new JobOperationAuditEntity { AuditId=Guid.NewGuid().ToString("N"), JobId=job.JobId,
                Operation="RequestSubjectCollection", PreviousStatus=AgentJobStatus.Pending, NewStatus=AgentJobStatus.Ready,
                ActorId=actor, Reason=payload.Name+" / "+jobType, CreatedAt=now });
