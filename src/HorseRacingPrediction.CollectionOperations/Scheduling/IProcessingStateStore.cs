...
public interface IProcessingStateStore
{
    ...
    Task<int> RequeueReadyCollectionDispatchesAsync(DateTimeOffset now, CancellationToken cancellationToken = default);
    ...
    Task<ForceRequeueJobResult> RerunJobAsync(string jobId, DateTimeOffset expectedUpdatedAt, string actorId, string? reason, DateTimeOffset now, CancellationToken cancellationToken = default);
