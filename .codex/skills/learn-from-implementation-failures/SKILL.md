...
- A successful workflow conclusion does not close actionable annotations. Before final handoff, inventory terminal annotations for every required workflow, classify compiler/analyzer warnings, deprecated action runtimes, security notices, and runner migrations as `fixed`, `non-actionable with evidence`, or an explicit follow-up owner. Any warning introduced by the change or repeated across runs is a closure item; keep the change incomplete until it is fixed or proven unrelated and non-blocking. When a deprecated action runtime is reported, inventory every repository workflow using that action family, upgrade to a supported official major with existing inputs preserved, and rerun all affected workflows.
