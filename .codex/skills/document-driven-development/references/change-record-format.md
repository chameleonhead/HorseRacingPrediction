...
At design time, `Owner` names the planned agent role, not just an unspecified implementer. `Model tier` and `Routing` (or adjacent prose for a short plan) explain why that role fits, who runs tests and accepts results, exclusive file ownership, parallel/serialized dependencies, and escalation conditions. Keep concrete requested model/agent IDs for dispatch and observed telemetry for execution; unstarted plans must not imply measured results. A single short lead-owned task can state that directly without creating subagents or a separate audit table.

If the requested outcome includes implementation or recovery, enumerate the path through code, tests, deployment/operation when applicable, and independent outcome verification. An unresolved cause stays an incomplete task even when a linked child record owns its design. Do not replace resolution criteria with merely assigning a task; preserve explicit operation and destructive-action approval gates.
