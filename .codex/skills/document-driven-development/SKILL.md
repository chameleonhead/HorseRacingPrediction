...
Before requesting design approval, make agent assignment actionable in that plan: name the planned agent role for each task (including lead-only tasks), its capability tier and selection reason, exclusive write scope, dependencies, verification responsibility, and handoff/escalation boundary. Do not defer these choices behind a generic `worker` or `task owner` placeholder. Resolve concrete model IDs at dispatch and record them separately from observed model telemetry; a design-time role is not evidence that an agent ran. A short lead-only change needs only a compact assignment and non-delegation reason, not mandatory subagents. Use `agent-task-orchestration` for delegation details rather than duplicating its audit schema.

When the user defines the change through implementation or operational resolution, keep that outcome in the governing record. Investigation, child-record creation, PR merge, and deployment are milestones, not substitutes for the requested completion. Unknown causes remain owned incomplete tasks with evidence and design/authorization gates; moving them into a child record does not exclude them from parent acceptance. Continue independent approved slices while blocked slices wait, without expanding authority for data repair or production operations.
...
- When a completion/result crosses separately declared producer and consumer DTOs, map every acceptance-relevant field (including optional evidence) through serialization, deserialization, persistence, and read-back. Use a production-shaped payload preserving repeated stages or items for the same entity; a one-item synthetic payload or direct-store test cannot prove that contract. Verify first-write and partial-failure semantics without deriving missing evidence from aggregate success.
