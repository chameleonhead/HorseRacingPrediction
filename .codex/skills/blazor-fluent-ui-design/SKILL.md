...
ここでいう文書化不要はImplementation Noteを省略できるという意味に限る。
AGENTS.mdと`document-driven-development`が要求するchange record、受入れ基準、
検証記録はUI変更の規模にかかわらず省略しない。適用中のchange recordがある場合、
このスキルは画面目的、情報階層、操作、状態、responsive/accessibilityに関する
設計判断を、該当するACと設計判断の記録へ接続する責務を持つ。

設計責務はこのスキルにあり、テスト結果やブラウザー観測の完了証拠は
`blazor-ui-testing`へ委ねる。判断をworkerへ委譲する場合も、主担当は設計適合をレビューする。
